...
// --- NEWS SECTION COMPONENT EXTRACTED FROM src/components/sections/NewsSection.tsx ---
...
function categoryIcon(cat: string) {
  ...
  if (c.includes('dato') || c.includes('sab')) return Newspaper;
  return Sparkles;
}
...
function NewsSectionComponent() {
  ...
  // Load from Sheet when admin logs in
  useEffect(() => {
    if (!token) {
      setNews(FALLBACK);
      return;
    ...
    getRows(SHEET, token)
      .then((rows) => {
        setNews(rows as unknown as NewsItem[]);
      })
      .catch(console.error);
  }, [token]);
  ...
  const handleChange = (row: number, field: keyof NewsItem, value: string) => {
    ...
    const timer = setTimeout(async () => {
      if (!token) return;
      const item = news.find((n) => n._row === row);
      if (!item) return;
      const updated = { ...item, [field]: value };
      const { _row, ...data } = updated;
      setSaving(row);
      await updateRow(token, _row, data as Record<string, string>, SHEET).catch(console.error);
      setSaving(null);
    }, 800);
    ...
    toast('¿Eliminar esta nota?', {
      action: {
        ...
        onClick: async () => {
          ...
          if (fresh.length > 0) setNews(fresh as NewsItem[]);
        },
      },
      cancel: {
        label: 'Cancelar',
...
// --- END NEWS SECTION COMPONENT ---

const categories = ["Todos", "Días Célebres", "Reconocimientos", "Datos Curiosos", "Empresa"]
  ...
  return (
    <main className="bg-background">
      ...
      <PageHeader
        ...
        backgroundImage="https://images.unsplash.com/photo-1504711432863-1ec85cba7bb9?auto=format&fit=crop&q=80&w=1920"
      />

      {/* Filter pills */}
      <section className="pt-12 pb-0 bg-gray-50 dark:bg-gray-900/50">
        <div className="container mx-auto px-4">
